...
-- Le cycle minimum de la chaudière est de 5min pour réduire l'usure -
----------------------------------------------------------------------
...
local CycleMini = 300                      --Durée de chauffe minimum en secondes (fonction de la chaudière)
local InterRelais = 'Chaudiere'            --Sortie vers relais chaudière
...
if (uservariables['DEBUG']=="false") then
  Debug = 0
  print ("--  debug = " .. Debug)
else
  Debug = 1
...
if( (minutes==15) or (minutes==0) or (minutes==30) or (minutes==45) ) then
   ...
   if  OnOff == 'On' then  --Si l'inter général est ON
      ...
      if Cycle < CycleMini then --Si le cycle est trop court on coupe
         ...
         --DB-- commandArray['Variable:'..VarCycle]= tostring('0')
      ...
      elseif Cycle > 870 then --Si le cycle est trop long (> 14min30) on allume sans coupure
         ...
         --DB-- commandArray['Variable:'..VarCycle]= tostring(Cycle)
