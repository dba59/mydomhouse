...
--------------------------------
local VarConsigne =  "Thermostat"                  --Température de consigne
local sonde = 'TeHu_Salon'                         --Nom de la sonde de température
local IDerreur = '167'                             --IDX de l'afficheur d'erreur
local IDSomerreur = '165'                          --IDX de l'afficheur Somme d'erreur
local IDDeltaErreur = '166'                        --IDX de l'afficheur Différence d'erreur
local IDCmdChauff = '168'                          --IDX de l'afficheur Proportionnel Commande Chauffage
local domoticz_url = '192.168.0.25:8081'            --Adresse IP:port de Domoticz
...
local fichier = '/var/tmp/RAM_FILE/dom_tmperr.txt' --Chemin d'acces du fichier sur un RAMdisque pour réduire l'usure de la carte SD
...
if (devicechanged[sonde]) then
  ...
  --Suppression de l'élement le plus vieu
  while #t > nbr_stk do
      table.remove(t,1)
  end
    -- Eriture dans fichier'
  f = io.open(fichier,"w")
